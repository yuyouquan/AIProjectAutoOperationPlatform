...
1. AI 自动巡检演示台：点击“开始 AI 自动巡检”，自动播放读取数据、原始数据解析、AI 思考过程、决策输出、生成任务、模拟写入和输出报告。
2. 数据源总览：项目管理、缺陷、体验大数据、售后与 VOC 四类信号。
3. AI 推理链路：收集、理解、判断、生成、跟踪。
4. 节点风险雷达：按 CAMON / tOS 样例项目识别节点风险。
5. AI 任务生成：把风险转成带责任、截止时间、证据和验收口径的任务建议。
6. 自治跟踪看板：模拟创建任务后，看板与报告同步更新。
7. 周会报告输出：生成面向项目周会和管理层的中文摘要。
8. 参赛要求覆盖：说明一页纸方案、Demo 1.0、既有立项增量、业务价值和可落地性。
